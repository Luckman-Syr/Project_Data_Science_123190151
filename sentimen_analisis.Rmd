...
```{r load dataset}
d <- vroom(here("D:/Project_DS/data_sicepat.csv"))
komen <- d$x
...
d<-read.csv("dataclean.csv",stringsAsFactors = FALSE) 
...
#skoring
katapositif <- scan("positive-words.txt",what="character",comment.char=";")
katanegatif <- scan("negative-words.txt",what="character",comment.char=";")
kata.positif = c(katapositif, "bagus","suka","senang","nice","mantap")
kata.negatif = c(katanegatif, "taik","anjing","payah","susah","kecewa")
...
twitter<- read.csv("D:/Project_DS/datalabel.csv")
tweet<- twitter$text
ui <- fluidPage(
    titlePanel("Sentiment Analysis dari komen atau review pengguna Sicepat"),
        mainPanel(
...
server <- function(input, output) {
    ...
    output$scatterplot <- renderPlot({produk_dataset<-read.csv("D:/Project_DS/dataclean.csv",stringsAsFactors = FALSE)
...
par(mar=rep(3,4))
barplot(colSums(s),col=c('gray'),ylab='count',main='Sentiment Analysis SiCepat')
    }, height=400)
